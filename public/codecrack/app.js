...
console.log(lowerBound([1,3,3,5,7], 3));`,options:["1","2","3","0"],answer:0,explanation:"Lower bound finds first position where value >= target. Index 1 has the first 3."},{type:"bugspot",topic:"arrays-hashing",difficulty:"easy",prompt:"This should return the sum of an array. What's the bug?",code:`function sum(arr) {
  ...
  return result;
}`,options:["Stack should store temps not indices","result[idx] should be i - idx","while should use <=","Should iterate right to left"],answer:1,explanation:"Days to wait = i - idx (future minus past), not idx - i which gives negative."}];var d="codecrack.leaderboard",q="codecrack.name",Y=10,p="easy",m=120,t;function C(e){let n=[...e];for(let i=n.length-1;i>0;i--){let o=Math.floor(Math.random()*(i+1));[n[i],n[o]]=[n[o],n[i]]}return n}function _(e){return C(S.filter(n=>n.difficulty===e))}function f(){return localStorage.getItem(q)||""}function K(e){localStorage.setItem(q,e)}function A(e,n){return`${e}-${n}`}function h(e,n){try{let i=localStorage.getItem(d);if(i)return JSON.parse(i)[A(e,n)]||[]}catch{}return[]}function J(e,n,i){let o;try{let s=localStorage.getItem(d);o=s?JSON.parse(s):{}}catch{o={}}let r=A(e,n);o[r]||(o[r]=[]),o[r].push(i),o[r].sort((s,z)=>z.score-s.score),o[r]=o[r].slice(0,Y),localStorage.setItem(d,JSON.stringify(o))}var a=e=>document.getElementById(e),N=a("screen-home"),g=a("screen-game"),H=a("screen-over"),$=document.querySelectorAll(".diff-btn"),X=a("btn-120"),Z=a("btn-180"),tt=a("btn-300"),j=document.querySelectorAll(".lb-tab"),T=a("home-lb-list"),L=a("home-lb-empty"),W=a("game-countdown"),c=a("countdown-fill"),et=a("game-score"),nt=a("game-streak"),ot=a("game-correct"),E=a("game-diff-label"),D=a("question-type"),it=a("question-prompt"),M=a("code-block"),at=a("code-content"),y=document.querySelectorAll(".option-btn"),u=a("explanation"),B=a("modal-name"),l=a("input-name"),I=a("btn-name-confirm"),rt=a("over-score"),st=a("over-correct"),ct=a("over-accuracy"),lt=a("over-streak"),pt=a("over-new-best"),ut=a("over-lb-list"),dt=a("btn-play-again");function P(e){let n=document.createElement("div");return n.textContent=e,n.innerHTML}function w(e){[N,g,H].forEach(n=>n.classList.add("hidden")),e.classList.remove("hidden")}function G(e){p=e,$.forEach(n=>n.classList.toggle("active",n.dataset.diff===e)),b(m)}function R(e,n,i){e.innerHTML=n.map((o,r)=>{let s=i&&o.score===i.score&&o.correct===i.correct&&o.name===i.name&&o.date===i.date;return`<li class="lb-entry ${r===0?"lb-gold":""} ${s?"lb-current":""}">
      <span class="lb-rank">${r+1}</span>
      <span class="lb-name">${P(o.name||"Anonymous")}</span>
      <span class="lb-score">${o.score}</span>
      <span class="lb-detail">${o.correct}/${o.total}</span>
      <span class="lb-date">${o.date}</span>
    </li>`}).join("")}function b(e){m=e,j.forEach(i=>i.classList.toggle("active",Number(i.dataset.mode)===e));let n=h(p,e);if(n.length===0){T.innerHTML="",L.classList.remove("hidden");return}L.classList.add("hidden"),R(T,n)}function ht(e){let n=f();n&&(l.value=n),B.classList.remove("hidden"),l.focus();function i(){let r=l.value.trim();r&&(K(r),B.classList.add("hidden"),I.removeEventListener("click",i),l.removeEventListener("keydown",o),e())}function o(r){r.key==="Enter"&&i()}I.addEventListener("click",i),l.addEventListener("keydown",o)}function F(e){t={difficulty:p,timeMode:e,questions:_(p),currentIndex:0,score:0,streak:0,bestStreak:0,correct:0,total:0,timeLeft:e,timerInterval:null,answered:!1},E&&(E.textContent=k[p]),w(g),V(),mt()}function x(e){f()?F(e):ht(()=>F(e))}function V(){t.currentIndex>=t.questions.length&&(t.questions=C([...t.questions]),t.currentIndex=0);let e=t.questions[t.currentIndex];t.answered=!1,D.textContent=v[e.type],D.className=`question-type type-${e.type}`,it.textContent=e.prompt,e.code?(M.classList.remove("hidden"),at.textContent=e.code):M.classList.add("hidden");let n=["A","B","C","D"];y.forEach((i,o)=>{i.className="option-btn",i.disabled=!1,i.innerHTML=`<span class="option-label">${n[o]}</span><span class="option-text">${P(e.options[o])}</span>`}),u.classList.add("hidden"),u.textContent="",U()}function Q(e){if(!t||t.answered||!t.timerInterval)return;t.answered=!0,t.total++;let n=t.questions[t.currentIndex],i=e===n.answer;if(i){let o=1+Math.floor(t.streak/3)*.25,r=Math.round(O[n.difficulty]*o);t.score+=r,t.streak++,t.correct++,t.streak>t.bestStreak&&(t.bestStreak=t.streak)}else t.streak=0;y.forEach((o,r)=>{o.disabled=!0,r===n.answer&&o.classList.add("correct"),r===e&&!i&&o.classList.add("wrong")}),u.textContent=n.explanation,u.classList.remove("hidden"),u.className=`explanation ${i?"explanation-correct":"explanation-wrong"}`,U(),setTimeout(()=>{t.timerInterval&&(t.currentIndex++,V())},i?1800:2500)}function mt(){t.timerInterval&&clearInterval(t.timerInterval);let e=t.timeMode;c.style.transition="none",c.style.width="100%",c.offsetWidth,c.style.transition="width 1s linear",t.timerInterval=window.setInterval(()=>{t.timeLeft--,c.style.width=`${t.timeLeft/e*100}%`,c.classList.toggle("timer-danger",t.timeLeft<=10);let o=Math.floor(t.timeLeft/60),r=t.timeLeft%60;W.textContent=`${o}:${r.toString().padStart(2,"0")}`,t.timeLeft<=0&&ft()},1e3);let n=Math.floor(t.timeLeft/60),i=t.timeLeft%60;W.textContent=`${n}:${i.toString().padStart(2,"0")}`}function ft(){t.timerInterval&&(clearInterval(t.timerInterval),t.timerInterval=null);let n={name:f()||"Anonymous",score:t.score,correct:t.correct,total:t.total,streak:t.bestStreak,date:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric"})},i=h(t.difficulty,t.timeMode),o=i.length>0?i[0].score:0,r=t.score>o&&t.score>0;J(t.difficulty,t.timeMode,n),rt.textContent=`${t.score}`,st.textContent=`${t.correct}/${t.total}`,ct.textContent=t.total>0?`${Math.round(t.correct/t.total*100)}%`:"0%",lt.textContent=`${t.bestStreak}`,pt.classList.toggle("hidden",!r);let s=h(t.difficulty,t.timeMode);R(ut,s,n),w(H)}function U(){et.textContent=`${t.score}`,nt.textContent=`${t.streak}`,ot.textContent=`${t.correct}`}function gt(){return!!t&&t.timerInterval!==null&&!g.classList.contains("hidden")}document.addEventListener("keydown",e=>{if(!gt()||t.answered)return;let n=parseInt(e.key);n>=1&&n<=4&&(e.preventDefault(),Q(n-1))});y.forEach((e,n)=>e.addEventListener("click",()=>Q(n)));X.addEventListener("click",()=>x(120));Z.addEventListener("click",()=>x(180));tt.addEventListener("click",()=>x(300));dt.addEventListener("click",()=>{b(m),w(N)});j.forEach(e=>{e.addEventListener("click",()=>b(Number(e.dataset.mode)))});$.forEach(e=>{e.addEventListener("click",()=>G(e.dataset.diff))});G("easy");})();
